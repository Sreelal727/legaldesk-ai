...
const caseOptions = [
  "All Cases",
  ...mockCases.map((c) => `${c.clientName} — ${c.caseNumber}`),
];

const caseOptionsNoAll = mockCases.map(
...
export const quickActionConfigs: QuickActionConfig[] = [
    ...
    form: {
      ...
      buildPrompt: (v) => {
        if (v.case && v.case !== "— New Client —") {
          return `Draft a Vakalatnama for the case: ${v.case}`;
        }
  ...
  },
  {
    ...
    form: {
      ...
      buildPrompt: (v) =>
        `Draft a bail application for ${v.accusedName}, Crime No. ${v.crimeNumber}, charged under sections ${v.sections}, to be filed in ${v.court}`,
    },
  },
  {
  ...
  },
  {
    ...
    form: {
      ...
      buildPrompt: (v) =>
        `Generate a client intake questionnaire for a new ${v.caseType} case`,
    },
  },
  {
    label: "📨 Follow-up",
    ...
    form: {
      ...
      buildPrompt: (v) =>
        `Draft a follow-up letter to the client in case ${v.case}. Purpose: ${v.purpose}`,
    },
  },
  {
    label: "🧾 Fee Receipt",
