...
function drawHeader(doc: jsPDF) {
  ...
  doc.text(FIRM_NAME, centerX, 18, { align: "center" });

  doc.setFont("helvetica", "bolditalic");
  doc.setFontSize(9);
  doc.setTextColor(100, 100, 100);
  doc.text(FIRM_TAGLINE, centerX, 24, { align: "center" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(120, 120, 120);
  doc.text(FIRM_ADDRESS, centerX, 29, { align: "center" });
  doc.text(FIRM_CONTACT, centerX, 33, { align: "center" });
...
function drawFooter(doc: jsPDF, pageNum: number, totalPages: number) {
  ...
  doc.text("Confidential — Nair & Associates", 20, footerY);
...
export function exportAsPdf(content: string, filename: string = "document") {
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith("|") && i + 1 < lines.length && lines[i + 1]?.match(/^\|[\s-:|]+\|$/)) {
      ...
      checkPageBreak(20);
      y = drawTable(doc, headers, rows, y, margin, usableWidth, pageHeight, bottomMargin, headerHeight);
      continue;
    }
