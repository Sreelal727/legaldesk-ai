...
const DEFAULT_FIRM_CONTACT = "Phone: +91 484 2345678 | Email: office@nairassociates.in";
const BRAND_COLOR = "075E54";
const LIGHT_GREEN = "E8F5E9";
const LIGHT_GRAY = "F5F5F5";

function createHeader(firmName: string, firmTagline: string, firmAddress: string, firmContact: string): DocxHeader {
  return new DocxHeader({
    children: [
      ...
      new Paragraph({
        ...
        spacing: { after: 20 },
        children: [
          new TextRun({ text: firmAddress, size: 16, color: "666666" }),
        ],
      }),
      new Paragraph({
        alignment: AlignmentType.CENTER,
        spacing: { after: 60 },
        children: [
          new TextRun({ text: firmContact, size: 16, color: "666666" }),
        ],
      }),
      new Paragraph({
        border: {
...
function createFooter(firmName: string): DocxFooter {
  return new DocxFooter({
    children: [
      ...
      new Paragraph({
        ...
        children: [
          new TextRun({ text: `Confidential — ${firmName}`, italics: true, size: 14, color: "999999" }),
...
export async function exportAsDocx(content: string, filename: string = "document", profile?: FirmProfile) {
    ...
    sections: [
      {
        ...
        headers: { default: createHeader(firmName, firmTagline, firmAddress, firmContact) },
        footers: { default: createFooter(profile?.firmName ?? "Nair & Associates") },
